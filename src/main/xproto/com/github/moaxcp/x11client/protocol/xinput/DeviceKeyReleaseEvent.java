...
@Value
@Builder
public class DeviceKeyReleaseEvent implements XEvent, XinputObject {
  ...
  public static DeviceKeyReleaseEvent readDeviceKeyReleaseEvent(byte firstEventOffset,
      boolean sentEvent, X11Input in) throws IOException {
    DeviceKeyReleaseEvent.DeviceKeyReleaseEventBuilder javaBuilder = DeviceKeyReleaseEvent.builder();
  ...
  public static class DeviceKeyReleaseEventBuilder {
    public boolean isStateEnabled(@NonNull KeyButMask... maskEnums) {
      for(KeyButMask m : maskEnums) {
        if(!m.isEnabled(state)) {
          return false;
    ...
    public DeviceKeyReleaseEvent.DeviceKeyReleaseEventBuilder stateEnable(KeyButMask... maskEnums) {
      for(KeyButMask m : maskEnums) {
    ...
    public DeviceKeyReleaseEvent.DeviceKeyReleaseEventBuilder stateDisable(
