...
@Value
@Builder
public class RawKeyReleaseEvent implements XGenericEvent, XinputObject {
  ...
  public static RawKeyReleaseEvent readRawKeyReleaseEvent(byte firstEventOffset, boolean sentEvent,
      byte extension, short sequenceNumber, int length, short eventType, X11Input in) throws
      IOException {
    RawKeyReleaseEvent.RawKeyReleaseEventBuilder javaBuilder = RawKeyReleaseEvent.builder();
  ...
  public static class RawKeyReleaseEventBuilder {
    public boolean isFlagsEnabled(@NonNull KeyEventFlags... maskEnums) {
      for(KeyEventFlags m : maskEnums) {
        if(!m.isEnabled(flags)) {
          return false;
    ...
    public RawKeyReleaseEvent.RawKeyReleaseEventBuilder flagsEnable(KeyEventFlags... maskEnums) {
      for(KeyEventFlags m : maskEnums) {
    ...
    public RawKeyReleaseEvent.RawKeyReleaseEventBuilder flagsDisable(KeyEventFlags... maskEnums) {
      for(KeyEventFlags m : maskEnums) {
