...
class GenerateX11ProtocolTask extends DefaultTask {
    ...
    def writeSource() {
        xcbXmls.get().asFileTree
            .findAll {
                it.name.endsWith('xproto.xml') || it.name.endsWith('bigreq.xml') || it.name.endsWith('compolite.xml') || it.name.endsWith('damage.xml') || it.name.endsWith('dpms.xml') || it.name.endsWith('dri2.xml') || it.name.endsWith('ge.xml') || it.name.endsWith('record.xml') || it.name.endsWith('res.xml') || it.name.endsWith('screensaver.xml') || it.name.endsWith('shape.xml') || it.name.endsWith('sync.xml') || it.name.endsWith('xc_misc.xml') || it.name.endsWith('xevie.xml') || it.name.endsWith('xf86dri.xml') || it.name.endsWith('xf86vidmode.xml') || it.name.endsWith('xinerama.xml') || it.name.endsWith('xselinux.xml') || it.name.endsWith('xtest.xml')
            }
            .each {
