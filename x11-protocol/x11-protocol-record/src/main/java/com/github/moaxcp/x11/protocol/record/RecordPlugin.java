...
public class RecordPlugin implements XProtocolPlugin {
  ...
  public Optional<Byte> getMinorVersion() {
    return Optional.ofNullable((byte) 13);
  }
  ...
  public XRequest readRequest(byte majorOpcode, byte minorOpcode, X11Input in) throws IOException {
    ...
    if(minorOpcode == RegisterClients.OPCODE) {
      return RegisterClients.readRegisterClients(in);
    }
    if(minorOpcode == UnregisterClients.OPCODE) {
      return UnregisterClients.readUnregisterClients(in);
    }
    if(minorOpcode == GetContext.OPCODE) {
      return GetContext.readGetContext(in);
    }
    if(minorOpcode == EnableContext.OPCODE) {
      return EnableContext.readEnableContext(in);
    }
    if(minorOpcode == DisableContext.OPCODE) {
      return DisableContext.readDisableContext(in);
    }
    if(minorOpcode == FreeContext.OPCODE) {
      return FreeContext.readFreeContext(in);
    }
